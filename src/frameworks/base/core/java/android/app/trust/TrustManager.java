...
public class TrustManager {
    ...
    private static final String DATA_FACTORS = "factors";
    ...
    public boolean isGranted(int userId,AuthSet factors) {
        try {
            return mService.isGranted(userId,factors);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    
    public void grant(int userId,AuthSet factors) {
        try {
            mService.grant(userId,factors);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public Intent nextChallenge(int userId) {
        try {
            return mService.nextChallenge(userId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public AuthSet granted(int userId) {
        try {
            return mService.granted(userId);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
        ...
        try {
            ITrustListener.Stub iTrustListener = new ITrustListener.Stub() {
                ...
                public void onTrustChanged(AuthSet factors,int userId, int flags) {
                    Message m = mHandler.obtainMessage(MSG_TRUST_CHANGED, 0, userId,
                            trustListener);
                    m.getData().putParcelable(DATA_FACTORS,factors);
                    if (flags != 0) {
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
                case MSG_TRUST_CHANGED:
                    ...
                    AuthSet factors=msg.getData().getParcelable(DATA_FACTORS);
                    ((TrustListener)msg.obj).onTrustChanged(factors, msg.arg2, flags);
                    break;
    ...
    public interface TrustListener {
         ...
         * @param factors The currently granted factors.
         ...
         */
        default void onTrustChanged(AuthSet factors, int userId, int flags) {
        }
        ...
        public static AuthSet fromTrustFlags(int flags) {
        	return AuthSet.get(flags>>16);
        }
        public static int trustFlags(AuthSet set) {
        	return set.mask()<<16;
        }
    }
}
