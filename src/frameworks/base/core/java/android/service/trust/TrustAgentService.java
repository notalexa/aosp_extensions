...
public class TrustAgentService extends Service {
    ...
    private static final AuthSet DEFAULT_GRANTS=AuthSet.get(AuthType.Secret);
    ...
    public final void grantTrust(final CharSequence message, final long durationMs, @GrantTrustFlags final int flags) {
        grantTrust(DEFAULT_GRANTS,message,durationMs,flags);
    }

    /**
     * Call to grant trust on the device.
     *
     * @param granted the set of authentication types granted by this call
     * @param message describes why the device is trusted, e.g. "Trusted by location".
     * @param durationMs amount of time in milliseconds to keep the device in a trusted state.
     *    Trust for this agent will automatically be revoked when the timeout expires unless
     *    extended by a subsequent call to this function. The timeout is measured from the
     *    invocation of this function as dictated by {)}.
     *    For security reasons, the value should be no larger than necessary.
     *    The value may be adjusted by the system as necessary to comply with a policy controlled
     *    by the system or {@link DevicePolicyManager} restrictions. See {@link #onTrustTimeout()}
     *    for determining when trust expires.
     ...
     * @throws IllegalStateException if the agent is not currently managing trust.
     */
    public final void grantTrust(final AuthSet granted,final CharSequence message, final long durationMs,@GrantTrustFlags final int flags) {
        synchronized (mLock) {
            if (!mManagingTrust) {
            ...
            if (mCallback != null) {
                try {
                    mCallback.grantTrust(granted,message.toString(), durationMs, flags);
                } catch (RemoteException e) {
                ...
                mPendingGrantTrustTask = new Runnable() {
                    @Override
                    public void run() {
                        grantTrust(granted,message, durationMs, flags);
                    }
                };
            }
        }
    ...
    private final class TrustAgentServiceWrapper extends ITrustAgentService.Stub {
        ...
        @Override
        public Intent nextChallenge() {
             return TrustAgentService.this.nextChallenge();
        }
