...
public class KeyguardHostView extends FrameLayout implements SecurityCallback {
    ...
    private final KeyguardUpdateMonitorCallback mUpdateCallback =
        ...
        @Override
        public void onTrustChanged(boolean enabled, AuthSet factors, int userId, int flags) {
            if (flags==0||(!enabled)||(userId != KeyguardUpdateMonitor.getCurrentUser())) return;
            ...
            if (initiatedByUser || dismissKeyguard) {
                if (mViewMediatorCallback.isScreenOn() && (bouncerVisible || dismissKeyguard)) {
                    ...
                    dismiss(AuthType.Secret,false /* authenticated */, userId);
                } else {
    ...
    public boolean dismiss(int targetUserId) {
        return dismiss(AuthType.Secret,false, targetUserId);
    }
    ...
    @Override
    public boolean dismiss(AuthType factor, boolean auth,int targetUserId) {
        return mSecurityContainer.showNextSecurityScreenOrFinish(auth, targetUserId);
    }
    ...
    public void finish(boolean strongAuth, int targetUserId) {
    	finish(AuthType.Secret,strongAuth,targetUserId);
    }
    @Override
    public void finish(AuthType factor, boolean auth, int targetUserId) {
        ...
        if (mViewMediatorCallback != null) {
            if (deferKeyguardDone) {
                mViewMediatorCallback.keyguardDonePending(auth, targetUserId);
            } else {
                mViewMediatorCallback.keyguardDone(auth, targetUserId);
            }
        }
    }
