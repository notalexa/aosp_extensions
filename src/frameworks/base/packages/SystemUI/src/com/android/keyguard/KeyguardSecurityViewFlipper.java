...
public class KeyguardSecurityViewFlipper extends ViewFlipper implements KeyguardSecurityView {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        final View child = getChildAt(getDisplayedChild());
        if(child!=null) {
            final LayoutParams lp = (LayoutParams) child.getLayoutParams();
	        if (lp.maxWidth > 0) {
	            maxWidth = Math.min(maxWidth,lp.maxWidth);
	        } else {
			    maxWidth=Math.max(0,maxWidth+lp.maxWidth);
	        }
	        if (lp.maxHeight > 0) {
	            maxHeight = Math.min(maxHeight,lp.maxHeight);
	        } else {
			    maxHeight=Math.max(0,maxHeight+lp.maxHeight);
	        }
	        ...
	        setMeasuredDimension(width + wPadding, height + hPadding);
       	    ...
       	    setMeasuredDimension(wPadding, hPadding);
        }
    }
