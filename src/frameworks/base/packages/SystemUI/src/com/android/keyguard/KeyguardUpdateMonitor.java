...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
	private static final AuthSet BIOMETRIC=AuthSet.get(AuthType.Biometric);
    ...
    private SparseArray<AuthSet> mUserGrants = new SparseArray<>();
    ...
    @Override
    public void onTrustChanged(AuthSet factors, int userId,int flags) {
        checkIsHandlerThread();
        boolean enabled=factors.implies(getAuthLevel(userId));
        if(!enabled) {
        	flags=0;
        }
        mUserGrants.put(userId,factors);
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onTrustChanged(enabled,factors,userId,flags);
            }
        }
    }
    ...
    protected void onFingerprintAuthenticated(int userId) {
        ...
        AuthSet granted=mUserGrants.get(userId,AuthSet.NONE).enable(BIOMETRIC);
        mTrustManager.grant(userId,granted);
        // Race condition.
        mUserGrants.put(userId,granted);
    ...
    public boolean getUserCanSkipBouncer(int userId) {
    	// Bouncer handles strong auth OR SIM PINS/PUKS
        return hasUserTrust(userId) && !isSimPinSecure();// || getUserUnlockedWithBiometric(userId);
    }

    public boolean getUserHasTrust(int userId) {
        return !isTrustDisabled(userId) && hasUserTrust(userId);
    }
    ...
    public boolean getUserUnlockedWithBiometric(int userId) {
        ...
        return fingerprintOrFace && getUserHasTrust(userId);
    }
    ...
    public boolean isUnlockingWithBiometricAllowed() {
    	AuthSet factors=mTrustManager.granted(getCurrentUser());
    	return factors.enable(AuthType.Biometric).implies(getAuthLevel(getCurrentUser()));
    }

    public boolean isUserInLockdown(int userId) {
        return mStrongAuthTracker.getStrongAuthForUser(userId).getAuthFlags()
    ...
    public static class StrongAuthTracker extends LockPatternUtils.StrongAuthTracker {
        ...
        public boolean hasUserAuthenticatedSinceBoot() {
            int userId = getCurrentUser();
            return (getStrongAuthForUser(userId).getAuthFlags()
    ...
    protected void handleStartedWakingUp() {
        ...
        mTrustManager.grant(uid, mUserGrants.get(uid,AuthSet.NONE));
    ...
    protected KeyguardUpdateMonitor(Context context) {
        ...
        mSecurityModel=new KeyguardSecurityModel(this,mLockPatternUtils,context);
    ...
    private boolean hasUserTrust(int userId) {
    	return hasUserTrust(userId,getAuthLevel(userId));
    }
    
    private boolean hasUserTrust(int userId,AuthLevel level) {
    	return mUserGrants.get(userId, AuthSet.NONE).implies(level);
    }
    ...
    private void sendUpdates(KeyguardUpdateMonitorCallback callback) {
        ...
        callback.onTrustChanged(getCurrentUser());
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (mFpm != null && mFpm.isHardwareDetected()) {
            ...
            final int strongAuthFlags = mStrongAuthTracker.getStrongAuthForUser(userId).getAuthFlags();
    ...
    public boolean isLockScreenDisabled(int userId) {
    	return mSecurityModel.isLockScreenDisabled(userId);
    }

	public int getGrantedTrust(int userId) {
		AuthSet factors=mUserGrants.get(userId, AuthSet.NONE);//mTrustManager.granted(userId);
		return factors.getTrustWeight();
	}
	
	public int getRemainingTrustWeight(int userId) {
		AuthSet factors=mUserGrants.get(userId, AuthSet.NONE);//.granted(userId);
		return factors.getRemainingTrustWeight(getAuthLevel(userId));
	}
	
	public AuthLevel getAuthLevel(int userId) {
		return mStrongAuthTracker.getStrongAuthForUser(userId).getTrustLevel();//(userId);
	}

	public void reportSuccessfulAuthentication(int userId,AuthType factor) {
		mTrustManager.grant(userId, mUserGrants.get(userId,AuthSet.NONE).enable(AuthSet.get(factor)));
	}

	public SecurityMode getSecurityMode(int targetUserId) {
		return mSecurityModel.getSecurityMode(targetUserId);
	}
}
