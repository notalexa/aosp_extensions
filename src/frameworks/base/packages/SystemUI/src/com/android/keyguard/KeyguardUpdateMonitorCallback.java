...
public class KeyguardUpdateMonitorCallback {
    ...
    /**
     * Implements the old default implementation from {@link KeyguardUpdateMonitor#onTrustChanged(boolean, AuthSet, int, int)}.
     * 
     * @param enabled is trust enabled?
     * @param factors the factors granted
     * @param userId the userid of the user for which factors has granted
     * @param flags the user flags.
     */
    public void onTrustChanged(boolean enabled,AuthSet factors,int userId,int flags) {
        onTrustChanged(userId);
        if (enabled && flags != 0) {
            onTrustGrantedWithFlags(flags, userId);
        }
    }
