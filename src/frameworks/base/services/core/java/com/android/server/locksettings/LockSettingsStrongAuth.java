...
public class LockSettingsStrongAuth {
    ...
    private final SparseArray<StrongAuthFlags> mStrongAuthForUser = new SparseArray<>();
    private final ArrayMap<Integer, StrongAuthTimeoutAlarmListener>
            mStrongAuthTimeoutAlarmListenerForUser = new ArrayMap<>();
    private final StrongAuthFlags mDefaultStrongAuthFlags;
    ...
    private void handleAddStrongAuthTracker(IStrongAuthTracker tracker) {
        ...
        for (int i = 0; i < mStrongAuthForUser.size(); i++) {
            ...
            StrongAuthFlags value = mStrongAuthForUser.valueAt(i);
            try {
                tracker.onStrongAuthRequiredChanged(value.getLevel(),value.getAuthFlags(), key);
    ...
    private void handleRemoveUser(int userId) {
        ...
        if (index >= 0) {
            ...
            notifyStrongAuthTrackers(mDefaultStrongAuthFlags.getLevel(),mDefaultStrongAuthFlags.getAuthFlags(), userId);
        }
    }
    ...
    private void notifyStrongAuthTrackers(AuthLevel level,int strongAuthReason, int userId) {
        int i = mTrackers.beginBroadcast();
        try {
            while (i > 0) {
                i--;
                try {
                    mTrackers.getBroadcastItem(i).onStrongAuthRequiredChanged(level,
