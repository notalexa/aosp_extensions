...
public class TrustManagerService extends SystemService {
    ...
    private static final int MSG_GRANT_PERM_TO_USER = 16;
    ...
    private final Grants NO_GRANTS=new Grants(-1,AuthSet.NONE);
    ...
    @GuardedBy("mUserGrants")
    private final SparseArray<Grants> mUserGrants=new SparseArray<>();
    ...
    private Intent nextChallengeInternal(int userId) {
       for (int i = 0; i < mActiveAgents.size(); i++) {
            AgentInfo agent = mActiveAgents.valueAt(i);
            if (agent.userId == userId) {
                Intent intent=agent.agent.nextChallenge();
                if(intent!=null) {
                    return intent;
                }
            }
    ...
    private Grants getGrants(int userId) {
        Grants g=mUserGrants.get(userId);
        if(g==null) {
            g=new Grants(userId,AuthSet.NONE);
            mUserGrants.put(userId,g);
        }
        return g;
    }
    ...
    private final class SettingsObserver extends ContentObserver {
        ...
        private final boolean mAcceptEscrowTokens;
        ...
        SettingsObserver(Handler handler) {
            ...
            mAcceptEscrowTokens = packageManager.hasSystemFeature(PackageManager.FEATURE_ACCEPT_ESCROW_TOKENS);

            mContentResolver = getContext().getContentResolver();
            updateContentObserver();
        }
        
        boolean doAcceptEscrowTokens() {
            return mAcceptEscrowTokens;
        }
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (TRUST_AGENTS_EXTEND_UNLOCK.equals(uri)) {
                ...
                int defaultValue = mAcceptEscrowTokens ? 0 : 1;
   ...
   // Agent management
    ...
    private void revokeTrustAll() {
        synchronized(mUserGrants) {
        	int n=mUserGrants.size();
        	for(int i=0;i<n;i++) {
        		Grants grants=mUserGrants.valueAt(i);
        		grants.reset();
        		if(DEBUG) {
        		    Slog.d(TAG,"Revoke grant of userid "+mUserGrants.keyAt(i)+" to "+grants);
        		}
        	}
        }
    }
    ...
    private void updateTrust(int userId, int flags) {
        updateTrust(userId, flags, false /* isFromUnlock */);
    }

    public void updateAgentTrust(int userId, AuthSet set, int flags) {
    	// Indirect
    	...
    	AuthSet auths=aggregateAgentGrants(userId);
    	Grants grants=mUserGrants.get(userId,NO_GRANTS);
    	if(grants==NO_GRANTS) {
    		grants=new Grants(userId,auths);
    		mUserGrants.put(userId, grants);
    	}
    	grants.setAgentGrants(auths).dispatch(flags);
    }

    private void updateTrust(int userId, int flags, boolean isFromUnlock) {
        getGrants(userId).dispatch(flags);
    }
        ...
        for (UserInfo userInfo : userInfos) {
            ...
            for (ResolveInfo resolveInfo : resolveInfos) {
                ...
                if (!mStrongAuthTracker.canAgentsRunForUser(userInfo.id)) {
                    int flag = mStrongAuthTracker.getStrongAuthForUser(userInfo.id).getAuthFlags();
    ...
    private AuthSet aggregateAgentGrants(int userId) {
    	AuthSet set=AuthSet.NONE;
        for (int i = 0; i < mActiveAgents.size(); i++) {
            AgentInfo info = mActiveAgents.valueAt(i);
            if (info.userId == userId) {
            	if (info.agent.getGrants()!=null) {
            		set=set.enable(info.agent.getGrants());
                }
            }
        }
        return set;
    ...
    private final IBinder mService = new ITrustManager.Stub() {
		@Override
		public Intent nextChallenge(int userId) {
			return nextChallengeInternal(userId);
		}
		@Override
		public AuthSet granted(int userId) {
			return getGrants(userId).mAgentGrants;
		}
        @Override
        ...
        private void dumpUser(PrintWriter fout, UserInfo user, boolean isCurrent) {
            ...
            fout.print(": granted=" + mUserGrants.get(user.id,NO_GRANTS));
            fout.print(", trustManaged=" + dumpBool(aggregateIsTrustManaged(user.id)));
            fout.print(", deviceLocked=" + dumpBool(isDeviceLockedInner(user.id)));
            fout.print(", strongAuthRequired=" + dumpHex(
                    mStrongAuthTracker.getStrongAuthForUser(user.id).getAuthFlags()));
            fout.println();
            ...
            for (AgentInfo info : mActiveAgents) {
                ...
                fout.print(", trusted=" + info.agent.getGrants());
                fout.println();
                if (info.agent.getMessage()!=null) {
        ...
        public void revokeAll() {
            ...
            mHandler.obtainMessage(MSG_REFRESH_DEVICE_LOCKED_FOR_USER, UserHandle.USER_ALL,
                   1/* 0*/ /* arg2 */).sendToTarget();
        }

		@Override
		public void grant(int userId, AuthSet factors) throws RemoteException {
			mHandler.obtainMessage(MSG_GRANT_PERM_TO_USER,userId, 0,factors).sendToTarget();
		}

		@Override
		public boolean isGranted(int userId, AuthSet factors) throws RemoteException {
	        long identity = Binder.clearCallingIdentity();
	        try {
	        	Grants grants=TrustManagerService.this.mUserGrants.get(userId,NO_GRANTS);
	        	return grants.mDispatchedGrants.containsAll(factors);
	        } finally {
	            Binder.restoreCallingIdentity(identity);
	        }
		}
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_GRANT_PERM_TO_USER:{
	                	...
	                	grants.enable((AuthSet)msg.obj);
	                	updateTrust(msg.arg1,0);
	                	break;
	                }
            }
        }
    ...
    private class Grants {
    	private AuthSet mDispatchedGrants;
    	private AuthSet mAgentGrants;
    	private AuthSet mKeyguardGrants;
    	private int mUserId;
    	private Grants(int userId,AuthSet auth) {
    		mUserId=userId;
    		this.mAgentGrants=auth;
    		this.mKeyguardGrants=mDispatchedGrants=AuthSet.NONE;
    	}

		public Grants setAgentGrants(AuthSet auths) {
    		this.mAgentGrants=auths;
			return this;
		}

		public boolean dispatch(int flags) {
    		AuthSet current=mAgentGrants.enable(mKeyguardGrants);
    		boolean dispatch=!mDispatchedGrants.equals(current);
    		if(dispatch) {
    			if (DEBUG) {
    				Log.i(TAG, "onTrustChanged(" + false + ", " + mUserId + ", "+current+", 0x"
                        + Integer.toHexString(flags) + ")");
    			}
    			if(dispatch) {
	    			for (int i = 0; i < mTrustListeners.size(); i++) try {
	                    mTrustListeners.get(i).onTrustChanged(current,mUserId, flags);
	                } catch (DeadObjectException e) {
	                    Slog.d(TAG, "Removing dead TrustListener.");
	                    mTrustListeners.remove(i);
	                    i--;
    	...
    	void set(AuthSet grants) {
    		mKeyguardGrants=grants;
    	}
    	
    	public void reset() {
    		mKeyguardGrants=AuthSet.NONE;
    	}
    	
    	public boolean grantsTrust(AuthLevel level) {
    		return mDispatchedGrants.implies(level);
    	}
    	
    	public String toString() {
    		return mAgentGrants.enable(mKeyguardGrants)+" (initial="+mAgentGrants+",keyguard="+mKeyguardGrants+")";
    	}
    	
    	public void enable(AuthSet set) {
    		set(mKeyguardGrants.enable(set));
    	}
    	public void enable(AuthType type) {
    		set(mKeyguardGrants.enable(type));
    	}
    	public void disable(AuthSet set) {
    		set(mKeyguardGrants.disable(set));
    	}
    	public void disable(AuthType type) {
    		set(mKeyguardGrants.disable(type));
    	}
    }
}
